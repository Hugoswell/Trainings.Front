...
const TrainingInfo = (props) => {
    ...
    const trainingId = createRef();
    const pushToTraining = () => {
        push(`/training/${trainingId.current.value}`);
    }

    return (
        <div onClick={pushToTraining} className="mt-8 flex justify-around items-center h-16 light-grey-bg rounded-lg">
            <input readOnly ref={trainingId} className="hidden" value={props.training.id} />
